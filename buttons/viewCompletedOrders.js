...
module.exports = {
    async execute(interaction, client) {
        try {
            ...
            const orders = db.getOrdersByStatus('completed');
            let description = '';
            
            if (orders.length === 0) {
                description = '**لا توجد طلبات مكتملة حالياً**';
            } else {
            ...
            const ordersEmbed = new EmbedBuilder()
                ...
                .setTitle(`${Icons.CHECK} الطلبات المكتملة`)
                .setDescription(description)
                .setFooter({ text: `${orders.length} طلب مكتمل` })
                .setTimestamp();

            await interaction.editReply({ embeds: [ordersEmbed] });

        } catch (error) {
            console.error('خطأ في عرض الطلبات المكتملة:', error);
            try {
                const errorEmbed = new EmbedBuilder()
                    .setColor(Colors.DANGER)
                    .setTitle(`${Icons.CROSS} خطأ`)
                    .setDescription('حدث خطأ أثناء عرض الطلبات المكتملة.')
                    .setTimestamp();
